...
<body class="page-chart page-gas" style="--bg-layer: radial-gradient(1200px 700px at 50% 10%, rgba(255,210,120,0.14), transparent 60%), var(--bg); --wrap-max:1000px; --wrap-pad:22px 14px 30px;">
<div class="wrap">
  <div class="top">
    <div>
  ...
  <div class="card">
    ...
    <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.1); display: flex; gap: 24px;">
      <div><span style="font-size: 12px; color: rgba(255,255,255,0.6);">Average</span><div style="font-size: 16px; font-weight: 500;"><b id="avgv">--</b> kΩ</div></div>
      <div><span style="font-size: 12px; color: rgba(255,255,255,0.6);">Min</span><div style="font-size: 16px; font-weight: 500;"><b id="minv">--</b> kΩ</div></div>
      <div><span style="font-size: 12px; color: rgba(255,255,255,0.6);">Max</span><div style="font-size: 16px; font-weight: 500;"><b id="maxv">--</b> kΩ</div></div>
    </div>
  </div>
</div>
...
function getChartEndpoint(baseEndpoint, selectedDate) {
  if (selectedDate) {
    ...
    // e.g., /api/gas-date?date=2024-01-15
...
function loadAvailableDates() {
  fetch('/api/available-dates?metric=gas', {cache:'no-store'})
    .then(r => r.json())
    .then(data => {
