...
inline String pageGraphPressure() {
  String h;
  h.reserve(9000);
  ...
  h += "<div class='top'><div><div class='k'>Graph</div><div class='v'>Sea-level Pressure (hPa)</div></div>";
  ...
  h += "if(ymax-ymin<0.2){ymax=ymin+0.2}";
  ...
  h += "ctx.strokeStyle='rgba(96,165,250,.85)'; ctx.lineWidth=2; ctx.beginPath();";
  ...
  h += "ctx.fillText('min '+ymin.toFixed(2)+' hPa', pad, h-pad+24);";
  h += "ctx.fillText('max '+ymax.toFixed(2)+' hPa', pad+250, h-pad+24);";
  h += "}";
  h += "async function tick(){let r=await fetch('/api/history?m=pressure',{cache:'no-store'}); let j=await r.json(); draw(j);} ";
