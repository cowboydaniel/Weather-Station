...
inline String pageGraphHumidity() {
  String h;
  h.reserve(9000);
  ...
  h += "<div class='top'><div><div class='k'>Graph</div><div class='v'>Humidity (%)</div></div>";
  ...
  h += "ctx.strokeStyle='rgba(167,139,250,.85)'; ctx.lineWidth=2; ctx.beginPath();";
  ...
  h += "ctx.fillText('min '+ymin.toFixed(1)+' %', pad, h-pad+24);";
  h += "ctx.fillText('max '+ymax.toFixed(1)+' %', pad+220, h-pad+24);";
  h += "}";
  h += "async function tick(){let r=await fetch('/api/history?m=humidity',{cache:'no-store'}); let j=await r.json(); draw(j);} ";
