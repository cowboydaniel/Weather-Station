...
inline String pageGraphTemp() {
  ...
  h += "<div class='top'><div><div class='k'>Graph</div><div class='v'>Temperature (°C)</div></div>";
  h += "<div><a href='/'>Dashboard</a></div></div>";
  h += "<div class='card'><canvas id='c' width='1200' height='520'></canvas></div>";
  h += "<script>";

  // minimal canvas chart
  h += "const c=document.getElementById('c'),ctx=c.getContext('2d');";
  h += "function draw(points){";
  h += "ctx.clearRect(0,0,c.width,c.height);";
  h += "const pad=50,w=c.width,h=c.height;";
  ...
  h += "let xmin=Math.min(...xs), xmax=Math.max(...xs);";
  h += "let ymin=Math.min(...ys), ymax=Math.max(...ys);";
  h += "if(ymax-ymin<0.1){ymax=ymin+0.1}";
  h += "function X(t){return pad+(t-xmin)/(xmax-xmin)*(w-2*pad)}";
  h += "function Y(v){return h-pad-(v-ymin)/(ymax-ymin)*(h-2*pad)}";
  h += "ctx.globalAlpha=1; ctx.strokeStyle='rgba(255,255,255,.12)'; ctx.lineWidth=1;";
  h += "for(let i=0;i<6;i++){let y=pad+i*(h-2*pad)/5;ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(w-pad,y);ctx.stroke();}";
  h += "ctx.strokeStyle='rgba(125,211,252,.8)'; ctx.lineWidth=2;";
  h += "ctx.beginPath();";
  h += "let started=false;";
  h += "for(const p of points){if(p.v===null) continue; let x=X(p.t), y=Y(p.v); if(!started){ctx.moveTo(x,y);started=true;} else ctx.lineTo(x,y);}";
  h += "ctx.stroke();";
  h += "ctx.fillStyle='rgba(230,233,242,.9)'; ctx.font='14px ui-sans-serif,system-ui';";
  h += "ctx.fillText('min '+ymin.toFixed(2)+' °C', pad, h-pad+24);";
  h += "ctx.fillText('max '+ymax.toFixed(2)+' °C', pad+220, h-pad+24);";
  h += "}";
  h += "async function tick(){let r=await fetch('/api/history?m=temp',{cache:'no-store'}); let j=await r.json(); draw(j);} ";
  h += "tick(); setInterval(tick,1500);";
